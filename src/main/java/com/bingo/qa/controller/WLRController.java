...
public class WLRController {
    ...
    @RequestMapping(value = "/sort")
        ...
        Map<String,Integer> indegree = new HashMap<>(); //统计用户入度
        Map<String,Integer> outdegree = new HashMap<>(); //统计用户出度
        for (int i = 1; i <= commentnum; i++) {
            String ansid = commentDao.retuserid(i);
            String askid = questionDAO.retuserid(commentDao.retentityid(i));
            if(indegree.containsKey(ansid)){
                indegree.put(ansid,indegree.get(ansid)+1);
            }else{
                indegree.put(ansid,1);
            }
            if(outdegree.containsKey(askid)){
                outdegree.put(askid,outdegree.get(askid)+1);
            }else{
                outdegree.put(askid,1);
            }
        ...
        System.out.println("用户入度");
        Iterator<Map.Entry<String, Integer>> indu = indegree.entrySet().iterator();
        while (indu.hasNext()) {
            Map.Entry<String, Integer> entry = indu.next();
            System.out.println(entry.getKey()+"  "+entry.getValue());
        }

        System.out.println("用户出度");
        Iterator<Map.Entry<String, Integer>> chudu = outdegree.entrySet().iterator();
        while (chudu.hasNext()) {
            Map.Entry<String, Integer> entry = chudu.next();
            System.out.println(entry.getKey()+"  "+entry.getValue());
        }
